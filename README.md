# Haziq Kaisar Hawwari

**Software & IoT Developer** — building web platforms, embedded systems, and computer-vision experiments.

I work mostly with Laravel on the web side, and Python/OpenCV/MediaPipe for computer vision and gesture-based interaction. Recently I've also been building small IoT hardware projects and experimenting with AI-assisted development workflows.

📫 [haziqkaisar56@gmail.com](mailto:haziqkaisar56@gmail.com)

<!--
  README maintenance notes:
  - Add LinkedIn / portfolio links to the "Contact" section once available
  - Update the project table below whenever a repo is public or its status changes
  - Badge colors follow each technology's official brand color (shields.io)
-->

---

## Featured Projects

| Project | Description | Stack |
|---|---|---|
| **TiketIn** | Event ticketing platform with online checkout, Tripay payment gateway integration, QR-based e-tickets sent automatically on payment, an admin dashboard, and a gate-scanning app for event check-in. | Laravel · MySQL · Tailwind CSS |
| **Dream Order** | Point-of-sale system for cashiers, with a single-screen POS terminal, product management, a Kanban-style order queue (Pending → In Progress → Completed), and a sales analytics dashboard. | Laravel · MySQL |
| **Hand Gesture Hologram Interface** | Real-time hand-tracking application that spawns and manipulates holographic visual effects — grab, pinch, and snap gestures trigger different interactions. | Python · OpenCV · MediaPipe |
| **Automated Trash Guard** | IoT hardware project for automated waste detection and handling. | Arduino · Microcontrollers |

*(Repo links will be added here as each project is published.)*

---

## Tech Stack

**Languages & Frameworks**
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)

**Data**
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)

**IoT & Hardware**
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square&logo=arduino&logoColor=white)

**Tools**
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub Copilot](https://img.shields.io/badge/Copilot-000000?style=flat-square&logo=githubcopilot&logoColor=white)

---

## GitHub Stats

<img src="https://github-readme-stats.vercel.app/api?username=haziqKaisar&show_icons=true&theme=default&hide_border=true&hide_title=true" height="150" alt="GitHub stats" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=haziqKaisar&layout=compact&theme=default&hide_border=true&hide_title=true" height="150" alt="Top languages" />

---

## Contact

[![Email](https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:haziqkaisar56@gmail.com)
<!-- Add LinkedIn / Portfolio badges here once available -->
